...
if __name__=='__main__':
    ...
    def get_layer_runtimes(init_model, conflict_score, sa_size=64):
        ...
        for i in range(len(train_conflicts)):
            B, _, _, _ = conv_layers[i].weight.shape
    ...
    conflict_scores = [0, 0.05, 0.15]
    wss_times, dss_times = [], []
    for score in conflict_scores:
        ws_times, ds_times = get_layer_runtimes(model, score, sa_size=32)
        wss_times.append(ws_times)
        dss_times.append(ds_times)

    bar_width = 0.25
    # times = wss_times + dss_times
    times = [wss_times[0]] + [dss_times[0]] + [dss_times[2]]
    # names = ['W-0', 'W-0.05', 'W-0.15', 'D-0', 'D-0.05', 'D-0.15']
    names = ['Weight Stationary', 'Data Stationary', 'Data Stationary(combine)']
    x_ticks = np.arange(len(times[0]))
    for i in range(len(times)):
        pos = x_ticks + i*bar_width
        plt.bar(pos, times[i], width=bar_width, edgecolor=None, label=names[i])
    
    plt.title('ShiftNet-19 Layerwise Runtime')
    plt.xlabel('Layer Index')
    plt.ylabel('Number of Cycles')
    plt.xticks(x_ticks + 1*bar_width, x_ticks+1)
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    plt.legend(loc=0)
    plt.savefig('figures/bar_times.png', dpi=300)
    plt.clf()
