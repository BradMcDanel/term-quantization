...
for name in names:
    ...
    for key, value in model_results.items():
        ...
        label = '{} ({})'.format(fancy_names[name], 'HESE')
...
for name in names:
    with open('data/{}-binary-results.txt'.format(name), 'r') as fp:
        model_results = json.load(fp)

    for key, value in model_results.items():
        if key == 'fixed': continue
        label = '{} ({})'.format(fancy_names[name], 'Binary')
        # color = colors['{}-{}'.format(name, key)]
        color = 'b'
        linestyle = line_styles['{}-{}'.format(name, key)]
        terms, accs = [], []
...
plt.savefig('figures/hese-vs-binary-accuracy.png', dpi=300, bbox_inches='tight')
plt.clf()