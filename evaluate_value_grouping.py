...
if __name__=='__main__':
    ...
    values = [1]
    group_sizes = [2]
    for i, (group_size, value) in enumerate(zip(group_sizes, values)):
        if i == 0:
            results[name] = {'avg_terms': [], 'acc': []}

        qmodel = models.convert_value_model(model, 3, 1, value, group_size,
                                            3, 1, value, group_size,
        ...
        acc = acc.item()
        results[name]['avg_terms'].append((4*value) / group_size)
        results[name]['acc'].append(acc)
        print(group_size, value, acc, (4*value) / group_size)
    ...
    with open('data/{}-value-results.txt'.format(args.arch), 'w') as fp:
