...
if __name__=='__main__':
    ...
    for i, (group_size, term) in enumerate(zip(group_sizes, terms)):
        ...
        qmodel = models.convert_binary_model(model, 3, 1, term, group_size,
                                             3, 1, term, group_size, 10)
    ...
    for i, (group_size, term) in enumerate(zip(group_sizes, terms)):
        ...
        qmodel = models.convert_binary_model(model, 3, 1, term, group_size, 3, 1, term,
                                             group_size, 10)
        criterion = nn.CrossEntropyLoss().cuda()
        qmodel = torch.nn.DataParallel(qmodel).cuda()
        _, acc = util.validate(val_loader, qmodel, criterion, args, verbose=False)
    ...
    with open('data/{}-binary-results.txt'.format(args.arch), 'w') as fp:
